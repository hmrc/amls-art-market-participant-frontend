...
class TypeOfParticipantControllerSpec extends SpecBase with MockitoSugar {
  ...
  "TypeOfParticipant Controller" must {
    ...
    "return OK and the correct view for a GET" in {
      ...
      val request = FakeRequest(GET, typeOfParticipantRoute)

      val result = route(application, request).value

      val view = application.injector.instanceOf[TypeOfParticipantView]
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(TypeOfParticipantPage, TypeOfParticipant.values).success.value
